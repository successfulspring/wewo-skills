...
Every final Playwright Test evidence run must use Playwright's native HTML
reporter with `open: "never"`. Preserve every established repository reporter
and add HTML alongside it rather than replacing list, JUnit, JSON, dot, GitHub,
or another configured reporter. For example:
...
Apply the same additive rule to the repository's existing configuration shape
and preserve an intentionally configured HTML output folder. Avoid a final-run
CLI reporter override that drops configured reporters. The normal default
output is `playwright-report/index.html`; verify that file, or the configured
HTML output folder's `index.html`, was actually generated by the final run.

The single-worker `--reporter=list` recipe above remains valid for diagnosis,
...
8. run the relevant browser target and affected regression; for the final
   Playwright Test evidence run, preserve existing reporters and include the
   configured native HTML reporter;
9. verify the final Playwright HTML report exists at
   `playwright-report/index.html` or the repository's configured HTML output
   folder, then collect other configured native evidence such as terminal
   output, JSON/JUnit, Trace, screenshots, or necessary video;
10. sanitize artifacts, clean data, and map evidence to the source case.

The native Playwright HTML report is mandatory for final Playwright Test
execution only; it does not change evidence requirements for other Runners.
Trace, screenshots, and video remain optional native evidence when configured
and useful. Automatic retries are diagnostic; they do not erase the initial
