...
authorized inputs in those same roles; read older sources only under the
bounded context rule below.
...
```text
docs/wewo/<workspace-key>/<requirement-slug>/test-cases.md
```

Do not create a test plan or intermediate risk, coverage, automation, or audit
artifact. Keep the filename and generated requirement slug in English; workspace
components follow the resolved mapping. Use the requested language,
...
constraints under the existing source-authority rules. Check applicability,
...
synchronization. Implementation observations cannot become requirement oracles.
